...
$mdFiles = [];
foreach (glob(rex_addon::get('tier')->getPath('docs') . '/*.md') ?: [] as $file) {
...
$page = rex_be_controller::getPageObject('tier/docs');
...
if (null !== $page) {
    foreach ($mdFiles as $key => $mdFile) {
        ...
        $page->addSubpage(
            (new rex_be_page($key, rex_i18n::msg('tier_docs_' . $keyWithoudPrio)))
            ->setSubPath($mdFile)
            ->setHref('index.php?page=tier/docs&mdfile=' . $key)
            ->setIsActive($key == $currenMDFile),
        );
    }
}

echo rex_view::title($this->i18n('tier_title'));
...
$fragment = new rex_fragment();
$fragment->setVar('title', rex_i18n::msg('package_help') . ' ' . rex_i18n::msg('tier_title'), false);
